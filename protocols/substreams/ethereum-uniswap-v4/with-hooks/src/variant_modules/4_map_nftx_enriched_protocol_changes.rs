    ...
    for tx_changes in &mut changes.changes {
        for component in &mut tx_changes.component_changes {
            ...
            if let Some(hook) = hook {
                if nftx_hooks.iter().any(|nftx| nftx == &hook) {
                    component
                        .static_att
                        .push(Attribute {
...
mod tests {
    ...
    const NFTX_HOOK: &str = "32d550542268d22ee1fd35d392780f4c729a2fcc";
    ...
    fn tags_only_nftx_hooks() {
        let block = block_with(vec![
            component_with_hook(NFTX_HOOK),
            component_with_hook(OTHER_HOOK),
        ]);
